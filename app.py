...
def index():
	url = get_url("stable")
	if not url: return "There was an error getting the latest stable NVDA version"
...
def alpha():
	...
	if not url: return "There was an error getting the latest NVDA alpha version"
	return redirect(url, code=301)
...
def beta():
	url = get_url("beta")
	if not url: return "There was an error getting the latest NVDA beta"
	return redirect(url, code=301)
